...
int __create_database(void* files, const char *filterExt, void **db, bool multi_core, bool pinyin_conversion)
{
    ...
    if (files!=NULL)
    {
        void *seg = mmseg_get_segmenter(mmsegManager);
        ...
        for (vector<pair<string,int> >::const_iterator it=vFiles->begin();it!=vFiles->end();it++)
        {
            ...
            dbDefault->IncludeTextFile(seg,it->first,strExts,it->second);
        }
    }
...
void loadDict();
extern "C" int idx_init_dict()
{
    loadDict();
    mmsegManager=mmseg_new_manager(".");
    printf("manager = %p\n",mmsegManager);
}
