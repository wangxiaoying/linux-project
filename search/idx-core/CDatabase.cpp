...
int CDatabase::IncludeTextFile(void *segmenter, const string& path,const string& exts,int boost)
{
    bool analyze=false;
	...
	CTextFile *clsFile=new CTextFile(res,boost,path,analyze,segmenter);
...
int CDatabase::SearchName(const vector<string> &vWords,CResultPool &cResult,const int idxDB)
{
    ...
    for (size_t k=0;k<vWords.size();k++)
    {
        ...
        {
            CTextFile *file = *it;
            string name = file->getLowerCaseFileName();
            size_t pos = name.find(vWords[k]);
            if (pos!=string::npos)
            {
                test.push_back(CResultPool::MakeRESID(idxDB,file->getFileID(),file->getBonus()),pos,vWords[k].length()-name.length());
            }else {
                name = file->getPinyinFileName();
                pos = name.find(vWords[k]);
                if (pos!=string::npos)
                {
                    test.push_back(CResultPool::MakeRESID(idxDB,file->getFileID(),file->getBonus()),pos,vWords[k].length()-name.length());
                }
...
{
    gzFile fp=gzopen(dbFile.c_str(),"wb");
...
int CDatabase::loadTrieTree(struct TRIENODE* node, gzFile& fp, const struct TRIENODE* parent, CDynVector<struct TRIENODE>* vNodes, CDynVector<struct RECORD>* vRecords)
{
	int cnt=0;
	assert(node!=NULL);
...
int CDatabase::saveTrieTree(const struct TRIENODE* node, gzFile& fp)
{
	int cnt=0;
	{
        NODE_SERIALIZATION d;
...
{
    gzFile fp=gzopen(dbFile.c_str(),"rb");
    if (NULL==fp) return false;
    LockTextFiles();
    DATABASE_SERIALIZATION d;
    d.numFiles=vFileList.size();
