...
CTextFile::CTextFile(const int id,const int score,const string& path,bool toprocess): \
...
void writeString(FILE* fp,string s)
{
    size_t len=s.length();
    fwrite(&len,sizeof(len),1,fp);
    fwrite(s.data(),len,1,fp);
}

void readString(FILE* fp,string& s)
{
    size_t len=0;
...
CTextFile::CTextFile(FILE* fp):m_WordData(NULL) // open the file here
{
    assert(fp!=NULL);
    TEXTFILE_SERIALIZATION d;
